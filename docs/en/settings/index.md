...
Holter splits settings into a per-account page and per-workspace pages:

| Page | Where to find it | Who can edit |
|------|------------------|--------------|
| [User](user.md) | **My account** in the workspace sidebar | The user themselves |
| [Workspace](workspace.md) | **Settings** in the workspace sidebar | Members with `Owner` or `Admin` role |
| [API tokens](api-tokens.md) | **API tokens** in the workspace sidebar | `Owner` only (admins can view) |
...
- A workspace page is at `/identity/workspaces/{workspace-slug}` and applies to that specific workspace.
- The API tokens page is at `/identity/workspaces/{workspace-slug}/api-tokens`.
