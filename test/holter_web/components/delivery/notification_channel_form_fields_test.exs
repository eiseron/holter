defmodule HolterWeb.Components.Delivery.NotificationChannelFormFieldsTest do
  ...
  describe "notification_channel_form_fields/1 — webhook type" do
    test "renders name input" do
      html = render_component(&notification_channel_form_fields/1, form: form(), selected_type: :webhook)
      assert html =~ "Name"
    end

    test "renders type select with webhook and email options" do
      html = render_component(&notification_channel_form_fields/1, form: form(), selected_type: :webhook)
      assert html =~ "Webhook"
      assert html =~ "Email"
    end
  ...
  describe "notification_channel_form_fields/1 — email type" do
    test "renders target as email input for email type" do
      html = render_component(&notification_channel_form_fields/1, form: form(), selected_type: :email)
  ...
  describe "notification_channel_form_fields/1 — locked type" do
    test "type select is disabled when locked_type is true" do
      html = render_component(&notification_channel_form_fields/1, form: form(), selected_type: :webhook, locked_type: true)
      assert html =~ "disabled"
    end

    test "type select is not disabled when locked_type is false" do
      html = render_component(&notification_channel_form_fields/1, form: form(), selected_type: :webhook, locked_type: false)
