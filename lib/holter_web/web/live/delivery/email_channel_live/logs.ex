defmodule HolterWeb.Web.Delivery.EmailChannelLive.Logs do
  use HolterWeb, :delivery_live_view

  import HolterWeb.LiveView.SortPagination

  alias Holter.Delivery.{ChannelLogs, EmailChannels}
  ...
  def handle_params(params, _uri, socket) do
    ...
    pagination = ChannelLogs.list_channel_logs(socket.assigns.channel, filters)
    form = to_form(Map.new(filters, fn {k, v} -> {Atom.to_string(k), v} end), as: "filters")
    path = ~p"/delivery/email-channels/#{socket.assigns.channel.id}/logs"
