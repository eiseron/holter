defmodule HolterWeb.Web.Delivery.NotificationChannelRecipientLive.Verify do
  ...
  def mount(%{"token" => token}, _session, socket) do
    result =
      if connected?(socket) do
        Delivery.verify_recipient(token)
      else
        Delivery.get_recipient_by_token(token)
      end
    ...
    case result do
      {:ok, recipient} ->
        {:ok,
         ...
         |> assign(:channel_id, recipient.notification_channel_id)}
