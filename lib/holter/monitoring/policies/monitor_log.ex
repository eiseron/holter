defmodule Holter.Monitoring.Policies.MonitorLog do
  @moduledoc """
  Authorization policy for `Holter.Monitoring.Models.MonitorLog`.

  Logs are written by check workers. Users only read them, scoped to
  ...
  @doc """
  Filters a MonitorLog query down to logs whose monitor lives in a
  workspace the user can access.
  """
  def scope(query, %User{} = user) do
    from l in query,
      join: m in Monitor,
      on: m.id == l.monitor_id,
