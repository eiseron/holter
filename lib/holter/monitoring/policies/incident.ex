defmodule Holter.Monitoring.Policies.Incident do
  @moduledoc """
  Authorization policy for `Holter.Monitoring.Models.Incident`.

  Incidents are written by the system. Users only read them, scoped to
  the parent monitor's workspace.

  | action  | minimum role |
  | ------- | ------------ |
  | `:read` | member       |

  Subject is the parent `%Monitor{}`; the boundary loads the monitor
  before calling the policy to avoid a hidden lookup inside this module.
  """
  ...
  @doc """
  Filters an Incident query down to incidents whose monitor lives in a
